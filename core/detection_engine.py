...
class DetectionEngine:
    ...
    def analyze_behavior(self, ip, duration, banner_sent):
        ...
        with self.lock:
            ...
            if 2 in self.modes:
                # Relaxed condition: Nmap often disconnects before banner is even sent or immediately after
                if duration < 1.0: 
                    # Check if this happened multiple times? Or just once is suspicious?
                    # For lab, 2+ quick drops is suspicious
                    short_conns = [c for c in record['connections'] if c['duration'] < 1.0]
                    if len(short_conns) >= 1: # Increased sensitivity for demo
